...
// Connect 监听ws服务
func (wss *WSServer) Connect(config *Config) {
...
func (wss *WSServer) any(w http.ResponseWriter, r *http.Request) {
	...
	// APICallers.Store(rsp.SelfID, c) // 添加Caller到 APICaller list...
	log.Infof("[wss] 连接Websocket服务器: %s 成功, 账号: %d", wss.Url, rsp.SelfID)
	wss.caller <- c
}

// Listen 开始监听事件
func (wss *WSServer) Listen(config *Config) {
	mux := http.ServeMux{}
	...
	for wssc := range wss.caller {
		go wssc.listen(config.processEvent)
	}
}
...
func (wssc *WSSCaller) listen(handler func([]byte,  APICaller)) {
	for {
		...
		if err != nil { // reconnect
			//	 APICallers.Delete(wssc.selfID) // 断开从apicaller中删除
		...
		if rsp.Get("echo").Exists() { // 存在echo字段，是api调用的返回
			log.Debug("[wss] 接收到API调用返回: ", strings.TrimSpace(tool.BytesToString(payload)))
		...
		log.Debug("[wss] 接收到事件: ", tool.BytesToString(payload))

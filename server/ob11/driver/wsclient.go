...
func (ws *WSClient) Connect() {
	...
	header := http.Header{
		...
		"User-Agent":    []string{"RosmBot/1.6.3"},
	}
	if ws.AccessToken != "" {
	...
	for {
		...
		APICallers.Store(ws.selfID, ws) // 添加Caller到 APICaller list...
...
// Listen 开始监听事件
func (ws *WSClient) Listen(handler func([]byte, APICaller)) {
	for {
		t, payload, err := ws.conn.ReadMessage()
		if err != nil { // reconnect
			APICallers.Delete(ws.selfID) // 断开从apicaller中删除
...
// CallApi 发送ws请求
func (ws *WSClient) GetSelfID() (id int64) {
	return ws.selfID
}

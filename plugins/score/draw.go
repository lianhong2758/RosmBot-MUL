...
func drawScore16(a *scdata) (image.Image, error) {
	// 绘图
	getAvatar, err := initPic(a.picfile, a.userPic)
	if err != nil {
		return nil, err
	}
	...
	if err := canvas.LoadFontFace(font.BoldFontFile, 50); err != nil {
		return nil, err
	}
	// draw head
	...
	if err := canvas.LoadFontFace(font.FontFile, 30); err != nil {
		return nil, err
	}
	...
	if err := canvas.LoadFontFace(font.BoldFontFile, 20); err != nil {
		panic(err)
	}
	canvas.DrawStringAnchored("Created By RosmBot", float64(imgDX)/2, float64(imgDY)-20, 0.5, 0.5) // zbp
	canvas.SetRGB255(0, 0, 0)
	canvas.DrawStringAnchored("Created By RosmBot", float64(imgDX)/2-3, float64(imgDY)-19, 0.5, 0.5) // zbp
	canvas.SetRGB255(255, 255, 255)
	// Gradient
...
func drawScore15(a *scdata) (image.Image, error) {
	// 绘图
	_, err := initPic(a.picfile, a.userPic)
	if err != nil {
		return nil, err
	}
	back, err := gg.LoadImage(a.picfile)
	...
	if err = canvas.LoadFontFace(font.BoldFontFile, float64(back.Bounds().Size().X)*0.1); err != nil {
		return nil, err
	}
	canvas.SetRGB(0, 0, 0)
	...
	if err = canvas.LoadFontFace(font.FontFile, float64(back.Bounds().Size().X)*0.04); err != nil {
		return nil, err
	}
...
func drawScore17(a *scdata) (image.Image, error) {
	...
	if err := canvas.LoadFontFace(font.MaokenFontFile, 24); err != nil {
		return nil, err
	}
	...
	canvas.SetRGB255(255, 255, 255)
	if err := canvas.LoadFontFace(font.MaokenFontFile, 20); err != nil {
		return nil, err
	}
	canvas.DrawStringAnchored("Created By RosmBot", float64(imgDX)/2, float64(imgDY)-20, 0.5, 0.5) // zbp
	canvas.SetRGB255(0, 0, 0)
...
func drawScore18(a *scdata) (img image.Image, err error) {
	fontdata, err := os.ReadFile(font.GlowSansFontFile)
	if err != nil {
		return
	}
	...
	go func() {
		defer wg.Done()
		awg.Wait()
		avatarbackimg = customrectangle(cw, ch, aw, ah, namew, color.NRGBA{R: 0, G: 0, B: 0, A: 70})
	}()
...
func customtext(a *scdata, fontdata []byte, cw, ch, aw float64, textcolor color.Color) (img image.Image, err error) {
	...
	canvas.DrawStringAnchored("Create By RosmBot", 0+4, ch, 0, -0.5)

...
// GetScoreByUID 取得分数
func (sdb *scoredb) GetScoreByUID(uid string) (s scoretable) {
	db := (*gorm.DB)(sdb)
...
func (sdb *scoredb) InsertOrUpdateScoreByUID(uid string, score int) (err error) {
	db := (*gorm.DB)(sdb)
	s := scoretable{
		UID:   uid,
		Score: score,
...
func (sdb *scoredb) GetSignInByUID(uid string) (si signintable) {
	db := (*gorm.DB)(sdb)
...
func (sdb *scoredb) InsertOrUpdateSignInCountByUID(uid string, count int) (err error) {
	db := (*gorm.DB)(sdb)
	si := signintable{
		UID:   uid,
		Count: count,
